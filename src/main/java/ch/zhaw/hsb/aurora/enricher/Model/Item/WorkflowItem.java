/*
* This file is part of the Aurora Enricher.
*
* (c) ZHAW HSB <apps.hsb@zhaw.ch>
*
* For the full copyright and license information, please view the LICENSE
* file that was distributed with this source code.
*/
package ch.zhaw.hsb.aurora.enricher.Model.Item;

/**
 * This class represents a specific item during the workflow in DSpace. Functions can be implemented in this class if needed.
 * 
 * @author Dana Ghousson ZHAW
 * @author Iris Hausmann ZHAW
 */
public class WorkflowItem extends ItemAbstract{

    private int claimedTaskID = -1;

    public int getClaimedTaskID() {
        return claimedTaskID;
    }

    public void setClaimedTaskID(int claimedTaskID) {
        this.claimedTaskID = claimedTaskID;
    }
    
}
